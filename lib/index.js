...
const genericMatcher = new RegExp(`\\b(\\d+(?:\\.\\d+)?(?:e[+-][0-9]+)?)\\s*((?:([kmgtpezy])(i?))?(b))\\b`);
...
function isParsable(input) {
  return (
    isBytes(input) || (Number.isFinite(input) && Math.abs(input) < 0x3fffffffffff90000000000) // Value above 1024 YiB
  );
}
