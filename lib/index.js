...
function isParsable(input) {
  return (
    isBytes(input) || (Number.isFinite(input) && Math.abs(input) < 0x40000000000000000000000) // All values 1024 YiB and under are parsable after sticky feature upgrade
  );
}
...
relative.size = (size, unit, options) => {
  ...
  let [result, prefixIndex, bits, iec] = [];
  if ((result = parseUnit(unit)) && ({prefixIndex, bits, iec} = result))
